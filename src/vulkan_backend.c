...
static void regenerate_framebuffers(void) {
  ...
  for (u32 i = 0; i < image_count; ++i) {
    ...
    // UI renderpass
    VkImageView ui_attachments[] = { context.swapchain.views[i] };
    VkFramebufferCreateInfo sc_framebuffer_create_info = {
      VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO,
      .renderPass = context.ui_renderpass.handle,
      .attachmentCount = 1,
...
b8 vulkan_renderer_backend_initialize(renderer_backend *backend, const char *application_name) {
  ...
  // Create swapchain and world framebuffers
  regenerate_framebuffers();
  ...
  if (!vulkan_material_shader_create(&context, &context.material_shader)) {
    KERROR("Failed to create the builtin material shader");
    return false;
  }
  if (!vulkan_ui_shader_create(&context, &context.ui_shader)) {
    KERROR("Failed to create the builtin UI shader");
    return false;
  }
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend *backend, f32 delta_time) {
  ...
  VkResult result = vkWaitForFences(context.device.logical_device,
                                    ...
                                    UINT64_MAX);
  if (!vulkan_result_is_success(result)) {
    KERROR("vulkan_renderer_backend_begin_frame :: `in_flight_fences` wait failed (%s)",
...
void vulkan_renderer_backend_update_world(Matrix4 proj,
  ...
  vulkan_material_shader_update(&context,
...
void vulkan_renderer_backend_update_ui(Matrix4 proj, Matrix4 view, i32 mode) {
  ...
  context.ui_shader.global_ubo.proj = proj;
  context.ui_shader.global_ubo.view = view;
  vulkan_ui_shader_update(&context,
                          &context.ui_shader,
                          context.frame_delta_time);
}
...
b8 vulkan_renderer_backend_end_frame(renderer_backend *backend, f32 delta_time) {
  ...
  if (context.images_in_flight[context.image_index] != VK_NULL_HANDLE) {
    VkResult result = vkWaitForFences(context.device.logical_device,
                                      ...
                                      UINT64_MAX);
    if (!vulkan_result_is_success(result)) {
      KFATAL("vulkan_renderer_backend_end_frame :: `images_in_flight` wait failed (%s)",
             vulkan_result_string(result, true));
    }
  }
...
b8 vulkan_renderer_backend_begin_renderpass(renderer_backend *backend, u8 renderpass_id) {
  ...
  case BUILTIN_RENDERPASS_UI:
    ...
    break;
  default:
    KERROR("vulkan_renderer_backend_begin_renderpass :: renderpass ID (%#02x) not valid",
           renderpass_id);
    return false;
  }
...
b8 vulkan_renderer_backend_end_renderpass(renderer_backend *backend, u8 renderpass_id) {
  ...
  case BUILTIN_RENDERPASS_UI:
    ...
    break;
  default:
    KERROR("vulkan_renderer_backend_end_renderpass :: renderpass ID (%#02x) not valid",
           renderpass_id);
    return false;
  }
...
void vulkan_renderer_backend_draw_geometry(geometry_render_data data) {
  ...
  case MATERIAL_TYPE_UI:
    vulkan_ui_shader_set_model(&context,
                               &context.ui_shader,
                               data.model);
    vulkan_ui_shader_apply_material(&context,
                                    &context.ui_shader,
                                    m);
    break;
  default:
    KERROR("vulkan_renderer_backend_draw_geometry :: unknown material type");
    return;
  }
...
b8 vulkan_renderer_backend_create_geometry(geometry *geometry,
  ...
  if (is_reupload) {
    ...
    old_range = (vulkan_geometry_data) {
      ...
      .index_element_size = internal_data->index_element_size,
      .vertex_buffer_offset = internal_data->vertex_buffer_offset,
      .vertex_count = internal_data->vertex_count,
      .vertex_element_size = internal_data->vertex_element_size
    };
  }
  else {
  ...
  if (is_reupload) {
    ...
    if (old_range.index_element_size) {
...
void vulkan_renderer_backend_destroy_geometry(geometry *geometry) {
  ...
  // Index data
  if (internal_data->index_element_size) {
    free_data_range(&context.object_index_buffer,
                    internal_data->index_buffer_offset,
...
b8 vulkan_renderer_backend_create_material(material *material) {
  ...
  case MATERIAL_TYPE_WORLD:
    if (!vulkan_material_shader_get_resources(&context,
                                              &context.material_shader,
                                              material)) {
      KERROR("vulkan_renderer_backend_create_material :: failed to get world shader resources");
      return false;
    }
    break;
  case MATERIAL_TYPE_UI:
    if (!vulkan_ui_shader_get_resources(&context,
                                        &context.ui_shader,
                                        material)) {
      KERROR("vulkan_renderer_backend_create_material :: failed to get UI shader resources");
      return false;
    }
    break;
  default:
    KERROR("vulkan_renderer_backend_create_material :: unknown material type");
    return false;
  }
...
void vulkan_renderer_backend_destroy_material(material *material) {
  ...
  case MATERIAL_TYPE_WORLD:
    ...
    break;
  case MATERIAL_TYPE_UI:
    vulkan_ui_shader_release_resources(&context,
                                       &context.ui_shader,
                                       material);
    break;
  default:
    KERROR("vulkan_renderer_backend_destroy_material :: unknown material type");
    return;
  }
}
